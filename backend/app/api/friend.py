...
@router.post("/request/{target_id}", response_model=rel_schemas.FriendshipResponse)
...
@router.post("/accept/{requester_id}", response_model=rel_schemas.FriendshipResponse)
async def accept_request(
    ...
    rel = await rel_crud.accept_friend_request(db, requester_id, addresse_id=current_user_id)
    if not rel:
        raise HTTPException(
...
@router.post("/reject/{requester_id}", status_code=status.HTTP_204_NO_CONTENT)
async def reject_request(
    ...
    success = await rel_crud.reject_friend_request(db, requester_id, addressee_id=current_user_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Pending request not found"
...
@router.post("/block/{target_id}", response_model=rel_schemas.FriendshipResponse)
